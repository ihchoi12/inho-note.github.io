...
- Initialize the library
  ...
  * set up communication between workers and allocate buffers
- Pin a GPU to each worker
  * typically each worker is using one of GPUs (run one process per GPU)
  * we wanna avoid contention from random GPU allocations
- Wrap the optimizer
